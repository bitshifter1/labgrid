...
class SSHDriver(CommandMixin, Driver, CommandProtocol, FileTransferProtocol):
    ...
    def _start_own_master_once(self, timeout):
        ...
        if self.networkservice.password:
            ...
            #with openssh>=8.4 SSH_ASKPASS_REQUIRE=force can be used
            #to get rid of the DISPLAY variable and the start_new_session flag
            env = {'SSH_ASKPASS': pass_file, 'DISPLAY':''}
            with open(fd, 'w') as f:
